...
4. For each sphere scanline, solve its X extent and Z depth using an integer square root.
...
6. Coalesce adjacent equal RGB565 pixels into horizontal rectangles, minimizing ABI calls without using private framebuffer pointers.
...
The largest zoom evaluates roughly 12,000 visible sphere pixels per frame. Each requires an integer square root, perspective divisions, procedural texture sampling, and light quantization. Identically colored neighbors are emitted as spans, keeping the public graphics-call count lower than a naïve pixel renderer while retaining a demanding CPU workload.
