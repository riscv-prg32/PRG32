...
/**
 * @brief Records a successfully presented frame and calculates the FPS.
 *
 * This function maintains an internal frame counter and an accumulation window.
 * When the window exceeds 1000 milliseconds, it calculates and updates the
 * globally displayed FPS value (`g_fps_value`), then resets the counter.
 * It must be called exactly once at the end of every successful frame
 * presentation.
 *
 * @param now_ms The current system time in milliseconds. Passed externally
 *               to ensure global synchronization and avoid redundant queries
 *               to the hardware timer.
 */
void prg32_band_note_frame(uint32_t now_ms) {

  // If this is the first time the function is called (window is 0)
  if (g_fps_window_ms == 0) {
    // Initialize the start of the timing window to the current timestamp
    g_fps_window_ms = now_ms;

    // Reset counter and fps value
    g_fps_frames = 0;
    g_fps_value = 0;

    // Exit early since we need to wait for a full window before calculating FPS
    return;
  }

  // Increment the counter because a new frame has just been successfully
  // presented
  g_fps_frames++;

  uint32_t elapsed = now_ms - g_fps_window_ms;

  // If 1000 milliseconds or more have passed, it's time to update the FPS
  if (elapsed >= 1000u) {
...
const char *prg32_band_render_text(uint8_t band, uint32_t now_ms) {
  ...
  case PRG32_BAND_MODE_WIFI: {
    ...
    snprintf(state->render, sizeof(state->render), "WIFI:%s IP:%s",
             ssid ? ssid : "", ip ? ip : "0.0.0.0");
    break;
  }
  case PRG32_BAND_MODE_GAME:
    if (g_game_info[0]) {
      ...
      if (prg32_cart_get_info(&info) == 0 && info.loaded) {
        snprintf(state->render, sizeof(state->render), "%s %.32s",
                 info.slot_name, info.name[0] ? info.name : "(unnamed)");
      } else {
    ...
    break;
  case PRG32_BAND_MODE_DEBUG:
    snprintf(state->render, sizeof(state->render), "%s",
