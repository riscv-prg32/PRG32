...
static int json_string_after(const char *start, const char *end, const char *key, char *out, size_t cap) {
    char needle[24];
...
static int json_arches_after(const char *start, const char *end, char *out, size_t cap) {
...
static int json_array_strings_after(const char *start, const char *end, const char *key, char *out, size_t cap) {
    char needle[24];
    snprintf(needle, sizeof(needle), "\"%s\"", key);
...
static int fetch_catalog(const char *base_url, char *status, size_t status_len) {
    ...
    if (!client) {
        ESP_LOGI(TAG,
                 "catalog client init failed heap=%lu",
                 (unsigned long)esp_get_free_heap_size());
    ...
    ESP_LOGI(TAG,
             "catalog fetch ok: status=%d content_len=%d bytes=%lu games=%d",
             http_status,
             content_len,
             (unsigned long)len,
             game_count);
    snprintf(status, status_len, truncated ? "first %d shown" : "OK", STORE_MAX_GAMES);
    return 0;
}
...
static int stream_download(const char *base_url, const store_game_t *game, uint8_t slot, char *status, size_t status_len) {
    char url[256];
    snprintf(url,
             sizeof(url),
             "%s/api/games/%s/download?architecture=%s&version=%s",
             base_url,
             game->id,
             current_arch(),
...
static void draw_store_list(int selected, int page, const char *note) {
    int pages = game_count > 0 ? (game_count + STORE_PAGE_SIZE - 1) / STORE_PAGE_SIZE : 1;
    char line[48];
    prg32_gfx_clear(PRG32_COLOR_BLACK);
    snprintf(line, sizeof(line), "BROWSE STORE  [page %d/%d]", page + 1, pages);
    prg32_gfx_text8(8, 8, line, PRG32_COLOR_WHITE, 0);
    prg32_gfx_text8(8, 26, "SEARCH", selected == -1 ? PRG32_COLOR_GREEN : PRG32_COLOR_CYAN, 0);
    int start = page * STORE_PAGE_SIZE;
    for (int i = 0; i < STORE_PAGE_SIZE && start + i < game_count; ++i) {
        const store_game_t *g = &games[start + i];
        int idx = start + i;
        snprintf(line, sizeof(line), "%c %-24s %.8s", idx == selected ? '>' : ' ', g->title, g->version);
        prg32_gfx_text8(8, 46 + i * 18, line, game_is_compatible(g) ? PRG32_COLOR_WHITE : PRG32_COLOR_YELLOW, 0);
    }
    if (note && note[0]) {
        prg32_gfx_text8(8, 190, note, PRG32_COLOR_YELLOW, 0);
    }
    prg32_gfx_text8(8, 216, "U/D SCROLL L/R PAGE SELECT DETAILS A BACK", PRG32_COLOR_CYAN, 0);
    prg32_gfx_present();
}
...
static int run_detail(const char *base_url, int index) {
    ...
    while (1) {
        char line[48];
        uint16_t action_color = game_is_compatible(g) ? PRG32_COLOR_CYAN : PRG32_COLOR_YELLOW;
        prg32_gfx_clear(PRG32_COLOR_BLACK);
        snprintf(line, sizeof(line), "%.24s  v%.10s", g->title, g->version);
        prg32_gfx_text8(8, 8, line, PRG32_COLOR_WHITE, 0);
        prg32_gfx_text8(8, 36, g->summary, PRG32_COLOR_CYAN, 0);
        snprintf(line, sizeof(line), "Architectures: %s", game_is_compatible(g) ? current_arch() : "(none)");
        ...
        if (!game_is_compatible(g)) {
            prg32_gfx_text8(8, 154, "NOT COMPATIBLE WITH THIS FIRMWARE", PRG32_COLOR_YELLOW, 0);
        }
        prg32_gfx_text8(8, 216, "U/D SLOT SELECT DOWNLOAD  A BACK", action_color, 0);
        prg32_gfx_present();
        ...
        if (input & PRG32_BTN_UP) {
            ...
            if (stream_download(base_url, g, slot, status, sizeof(status)) == 0) {
                ...
                while (1) {
                    ...
                    prg32_gfx_text8(8, 216, "SELECT RUN NOW  A BACK", PRG32_COLOR_CYAN, 0);
                    prg32_gfx_present();
