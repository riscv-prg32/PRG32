...
static int perf_buffers_alloc(void) {
  ...
  g_samples = heap_caps_calloc(PRG32_PERF_MAX_SAMPLES,
                               sizeof(prg32_perf_sample_t), MALLOC_CAP_8BIT);
  g_windows = heap_caps_calloc(PRG32_PERF_MAX_WINDOWS,
                               sizeof(prg32_perf_window_t), MALLOC_CAP_8BIT);
  if (!g_samples || !g_windows) {
...
static uint32_t percentile_sorted(const uint32_t *values, uint32_t count,
                                  uint32_t percentile) {
  if (!values || count == 0) {
    return 0;
  }
...
static void draw_progress(uint32_t frame, uint8_t screen_index) {
  ...
  snprintf(line, sizeof(line), "FRAME %lu / %lu", (unsigned long)frame,
...
static void scene_draw_text_overlay(uint8_t screen_index,
  ...
  static const char *const rows[] = {
      "REGISTER TRACE  A0 A1 A2 A3",     "MEMORY VIEW     0000 1000 2000",
      "STACK           RA SP S0 S1",     "FRAME BUDGET    33333 US",
      "WIFI            AP/STA METADATA", "CARTRIDGE       ABI PRG2",
  ...
  for (int y = 32; y < PRG32_GAME_H; y += 8) {
    ...
    if ((row & 3) == 0) {
      char value[32];
      snprintf(value, sizeof(value), "%04lx %04lx",
...
static void scene_draw_sprite_storm(uint8_t screen_index,
  ...
  static const uint16_t colors[] = {
      PRG32_COLOR_RED,    PRG32_COLOR_GREEN, PRG32_COLOR_BLUE,
...
static void compute_range_stats(prg32_perf_window_t *w, uint32_t window_index,
  ...
  w->duration_us =
      (uint32_t)(ended_us > started_us ? ended_us - started_us : 0);
  w->frames = count;
...
static void compute_window(uint32_t window_index, uint32_t first,
                           uint32_t last) {
...
static void compute_results(uint64_t started_us, uint64_t finished_us) {
  ...
  snprintf(g_summary.run_id, sizeof(g_summary.run_id), "perf-%llu-%lu",
           (unsigned long long)(started_us / 1000ULL),
           (unsigned long)g_run_sequence++);
  copy_text(g_summary.board_id, sizeof(g_summary.board_id),
            CONFIG_PRG32_METRICS_BOARD_ID);
  copy_text(g_summary.target, sizeof(g_summary.target), PRG32_PERF_TARGET_NAME);
  copy_text(g_summary.display_backend, sizeof(g_summary.display_backend),
            PRG32_PERF_DISPLAY_BACKEND);
  copy_text(g_summary.firmware_version, sizeof(g_summary.firmware_version),
            PRG32_FIRMWARE_VERSION);
  copy_text(g_summary.firmware_git_sha, sizeof(g_summary.firmware_git_sha),
            "unknown");
  copy_text(g_summary.build_type, sizeof(g_summary.build_type),
            PRG32_PERF_BUILD_TYPE);
  copy_text(g_summary.wifi_mode, sizeof(g_summary.wifi_mode),
            wifi_mode_name(prg32_wifi_current_mode()));
  copy_text(g_summary.game_name, sizeof(g_summary.game_name),
  ...
  g_summary.duration_us =
      (uint32_t)(finished_us > started_us ? finished_us - started_us : 0);
...
static void draw_summary(void) {
  ...
  snprintf(line, sizeof(line), "RUN: %.40s", g_summary.run_id);
  prg32_gfx_text8(8, 28, line, PRG32_COLOR_CYAN, 0);
  snprintf(line, sizeof(line), "OVERALL FPS: %lu.%02lu  SCREENS: %lu",
  ...
  snprintf(line, sizeof(line), "FRAME US MEAN/P95/P99: %lu / %lu / %lu",
  ...
  snprintf(line, sizeof(line), "UPDATE/DRAW/PRESENT: %lu / %lu / %lu",
  ...
  snprintf(line, sizeof(line), "MISSED: %lu/%lu  HEAP MIN: %lu",
...
int prg32_performance_test_run(void) {
  ...
  for (uint8_t screen_index = 0; screen_index < PRG32_PERF_SCREEN_COUNT;
    ...
    for (uint32_t local_frame = 0; local_frame < PRG32_PERF_SCREEN_FRAMES;
      ...
      prg32_perf_sample_t sample = {
          .frame_index = frame,
          .sampled_at_device_us = (uint64_t)present_end - lost_time_us,
      ...
      if (local_frame == 30) {
        int64_t frame_delay_start = esp_timer_get_time();
        vTaskDelay(1);
  ...
  uint64_t finished_us = (uint64_t)esp_timer_get_time() - lost_time_us;
...
static int stream_write(prg32_performance_json_writer_t writer, void *ctx,
                        const char *chunk) {
  if (!writer || !chunk) {
    return -1;
  }
...
static int stream_writef(prg32_performance_json_writer_t writer, void *ctx,
                         const char *fmt, ...) {
  char chunk[768];
  va_list ap;
  va_start(ap, fmt);
...
static int stream_json_string(prg32_performance_json_writer_t writer, void *ctx,
                              const char *value) {
...
static int stream_json_string_field(prg32_performance_json_writer_t writer,
                                    ...
                                    const char *value, int leading_comma) {
  if (stream_writef(writer, ctx, "%s\"%s\":", leading_comma ? "," : "", name) !=
      0) {
    return -1;
  }
...
static int stream_summary_fields(prg32_performance_json_writer_t writer,
                                 void *ctx) {
  return stream_writef(writer, ctx,
                       "\"frames\":%lu,"
                       "\"fps_mean\":%lu.%02lu,"
                       "\"frame_us_min\":%lu,"
                       "\"frame_us_mean\":%lu,"
...
int prg32_performance_json_write(prg32_performance_json_writer_t writer,
  ...
  if (stream_write(writer, ctx, "{\"ok\":true") != 0 ||
      stream_json_string_field(writer, ctx, "run_id", g_summary.run_id, 1) !=
          0 ||
      stream_json_string_field(writer, ctx, "board_id", g_summary.board_id,
                               1) != 0 ||
      stream_json_string_field(writer, ctx, "target", g_summary.target, 1) !=
          0 ||
      stream_json_string_field(writer, ctx, "display_backend",
                               g_summary.display_backend, 1) != 0 ||
      stream_json_string_field(writer, ctx, "firmware_git_sha",
                               g_summary.firmware_git_sha, 1) != 0 ||
      stream_json_string_field(writer, ctx, "firmware_version",
                               g_summary.firmware_version, 1) != 0 ||
      stream_json_string_field(writer, ctx, "game_name", g_summary.game_name,
                               1) != 0) {
    return -1;
  }
  if (stream_writef(writer, ctx, ",\"cartridge_generation\":%lu",
                    (unsigned long)g_summary.cartridge_generation) != 0 ||
      stream_json_string_field(writer, ctx, "build_type", g_summary.build_type,
                               1) != 0 ||
      stream_json_string_field(writer, ctx, "wifi_mode", g_summary.wifi_mode,
                               1) != 0 ||
  ...
  for (uint32_t i = 0; i < g_sample_count; ++i) {
    ...
    if (stream_writef(
            ...
            (unsigned long)s->min_free_heap_bytes, (unsigned long)s->input_mask,
  ...
  for (uint32_t i = 0; i < g_window_count; ++i) {
    ...
    if (stream_writef(
            ...
            "\"heap_min\":%lu"
            "}",
            i ? "," : "", (unsigned long)w->window_index,
            (unsigned long long)w->started_at_device_us,
            (unsigned long)w->duration_us, (unsigned long)w->frames,
            (unsigned long)(w->fps_mean_x100 / 100u),
            (unsigned long)(w->fps_mean_x100 % 100u),
            (unsigned long)w->frame_us_min, (unsigned long)w->frame_us_mean,
            (unsigned long)w->frame_us_p50, (unsigned long)w->frame_us_p95,
            (unsigned long)w->frame_us_p99, (unsigned long)w->frame_us_max,
