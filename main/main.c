...
static void prg32_configure_metrics(const char *game_name) {
  snprintf(g_metrics_game_name, sizeof(g_metrics_game_name), "%s",
...
static void prg32_record_frame_metric(
    uint32_t frame, uint32_t input_snapshot, int64_t frame_start_us,
    int64_t update_start_us, int64_t update_end_us, int64_t draw_start_us,
    int64_t draw_end_us, int64_t present_start_us, int64_t present_end_us) {
  ...
  while (1) {
    ...
    if (prg32_cart_is_loaded()) {
      ...
      if (prg32_metrics_is_enabled()) {
...
#if PRG32_DEBUG
        prg32_debug_overlay_draw(1, 0, 0, input_snapshot,
        ...
        prg32_gfx_unlock();
        prg32_record_frame_metric(prg32_diag_frame_count(), input_snapshot,
                                  frame_start_us, update_start_us,
                                  update_end_us, draw_start_us, draw_end_us,
                                  present_start_us, present_end_us);
      } else {
        prg32_gfx_lock();
        prg32_cart_call_update();
        prg32_cart_call_draw();
      ...
      if (now_ms - last_idle_log_ms >= PRG32_IDLE_HEARTBEAT_MS) {
        ...
        esp_rom_printf("PRG32 ROM: idle heartbeat input=0x%08" PRIx32 "\n",
                       input_snapshot);
        ESP_LOGI(TAG, "idle heartbeat: no cartridge loaded, input=0x%08" PRIx32,
                 input_snapshot);
      }
      prg32_gfx_present();
