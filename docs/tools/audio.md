...
### Synth ID Decoder Fix

Earlier firmware decoded the synth ID with a layout that did not match the
table above: resonance from bits 6:4, cutoff from bits 9:7, and pulse width
from bits 13:10. Cutoff could never exceed index 7 (about 300 Hz at 22050 Hz),
so every synth voice sounded muffled, and a resonance of 4..7 read past the
four-entry damping table. `PRG32_AUDIO_SYNTH_PULSE(8, 15, 0)`, for example,
decoded as resonance 6.

The firmware now decodes exactly the documented layout, and the filter masks
its table indices defensively. Instruments built with the
`PRG32_AUDIO_SYNTH_*` macros, `tools/prg32_audio_model.py`, or an equivalent
encoder (such as `cartridges/bachdemo`) now sound as designed. Cartridges
whose `sample_id` values were hand-tuned for the old decoder (for example the
SpaceBeltMadness `audio/sbm_audio.json` generated by its
`tools/compose_soundtrack.py`) will sound different and should re-encode their
instruments with the documented layout.

This is a behaviour fix, not an ABI change: no function index, prototype,
descriptor, event, or AUDIO block layout changed, so the ABI hash, version,
and feature bits stay the same (see
[ABI compatibility rules](../agents/cartridge_abi_compatibility.md)).
...
`delta_ticks` is the wait *after* an event, in tracker ticks (one tick is a
16th note: `60000 / (bpm * 4)` ms). Events separated by `delta_ticks = 0`
run in the same tick, so consecutive delta-0 `NOTE_ON` events form a chord.
Only waiting consumes time; see
[How `prg32_audio_play_track` Works](../software/audio_track_internals.md)
for the step algorithm.

Earlier firmware charged one tick for every event, even with
`delta_ticks = 0`, which turned chords into arpeggios and stretched every
track by one tick per event. Tracks that compensated for that (for example by
using delta 0 for "next 16th note") now play faster and should be re-timed.
